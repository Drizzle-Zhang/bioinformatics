...
# edgeR and heatmap
find.sig.KO <- function(df.meta, df.db.KEGG, path.plot, vec.dose, sub.time) {
    library(edgeR)
    library(gplots)
    ...
    factor.group <- as.factor(sel.meta$Gender)
    ...
    vec.sig[(res.edgeR$logFC > 1.5) & (res.edgeR$PValue < 0.01)] <- 1
    vec.sig[(res.edgeR$logFC < -1.5) & (res.edgeR$PValue < 0.01)] <- -1
    ...
    if (sum(vec.sig) > 0) {
        ...
        df.enrich$qvalue <- p.adjust(df.enrich$pvalue, method = 'fdr')
        df.enrich.sig <- df.enrich[df.enrich$qvalue < 0.05,]
        if (dim(df.enrich.sig)[1] > 1) {
            df.enrich.sig$lgp <- -log10(df.enrich.sig$pvalue)
            plot.deplete <- 
                ggplot(data = df.enrich.sig, aes(x = pathway, y = lgp)) + 
                geom_bar(stat = 'identity') +
                labs(y = '-log(Pvalue)', x = 'Pathway', title = 'Enriched in male') + 
                coord_flip()
            ggsave(filename = paste0("/pathway_treat_deplete_", sub.time, 
    ...
    if (sum(vec.sig) > 0) {
        ...
        if (dim(df.enrich.sig)[1] > 1) {
            ...
            plot.enrich <- 
                ggplot(data = df.enrich.sig, aes(x = pathway, y = lgp)) + 
                geom_bar(stat = 'identity') +
                labs(y = '-log(Pvalue)', x = 'Pathway', title = 'Enriched in female') + 
                coord_flip()
...
files.res <- foreach(sub.time = series.time, .combine = rbind) %dopar% 
    find.sig.KO(df.meta, df.db.KEGG, path.plot, vec.dose, sub.time)
