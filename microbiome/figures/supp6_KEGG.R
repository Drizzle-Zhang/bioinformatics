...
df.ks.male.filter <- 
    df.ks.male.filter[!(df.ks.male.filter$pathway %in% 
                            c('ABC transporters', 'Lipopolysaccharide biosynthesis',
                              'Cell cycle - Caulobacter', 'Ribosome', 
                              'Starch and sucrose metabolism', 'Glycolysis / Gluconeogenesis',
                              'Fructose and mannose metabolism', 'Translation factors',
...
df.ks.female.filter <- df.ks.female[
    df.ks.female$pvalue.control < 0.1 | df.ks.female$pvalue.treat < 0.1,]
