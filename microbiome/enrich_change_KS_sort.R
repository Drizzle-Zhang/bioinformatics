...
ggsave(filename = paste0("/Combine_Sum_GSEA_", mod, '_',
                         paste0(as.character(vec.dose), collapse = ''), ".png"),
       path = path.plot.male, plot = plot.male,
...
ggsave(filename = paste0("/Combine_Heatmap_GSEA_",  mod, '_',
                         paste0(as.character(vec.dose), collapse = ''), ".png"),
       path = path.plot.male, plot = plot.heatmap.male,
...
ggsave(filename = paste0("/Combine_Sum_GSEA_",  mod, '_',
                         paste0(as.character(vec.dose), collapse = ''), ".png"),
       path = path.plot.female, plot = plot.female,
       height = 15, width = 24, units = 'cm')
...
df.heatmap.female$pathway <- 
    factor(df.heatmap.female$pathway, 
           levels = as.character(df.ks.female.filter$pathway), ordered = T)
...
ggsave(filename = paste0("/Combine_Heatmap_GSEA_",  mod, '_',
                         paste0(as.character(vec.dose), collapse = ''), ".png"),
       path = path.plot.female, plot = plot.heatmap.female,
       height = 15, width = 25, units = 'cm')
