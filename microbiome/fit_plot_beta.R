...
ggplot(data = df.plot.male, aes(x = Time, y = Distance, color = Dose)) +
    ...
    geom_hline(yintercept = baseline.1, color = '#F8766D', linetype = 5) +
    geom_hline(yintercept = baseline.2, color = '#00BA38', linetype = 5) +
    geom_hline(yintercept = baseline.3, color = '#619CFF', linetype = 5) +
    theme(panel.background = element_rect(color = 'gray',
                                          fill = 'transparent')) +
    ylab('Silhouette Distance')
