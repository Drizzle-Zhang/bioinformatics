...
saveRDS(filtered_otutab, file = paste0('OTUs_filtered_', topN, '.Rdata'))
saveRDS(filtered_taxonomy, file = paste0('taxonomy_filtered_', topN, '.Rdata'))

# read Rdata
topN <- 500
setwd('/home/drizzle_zhang/microbiome/result/6.Phylogenetic/graphlan')
df.OTUs <- readRDS(paste0('OTUs_filtered_', topN, '.Rdata'))
df.taxonomy <- readRDS(paste0('taxonomy_filtered_', topN, '.Rdata'))

# delete unclassified families
idx.del <- df.taxonomy[, 'family'] %in% c('Unassigned', 'f__uncultured')
df.tax.filter <- df.taxonomy[!idx.del, ]
# output tree file
df.tree <- df.tax.filter[, c('phylum', 'class', 'order', 'family', 'genus')]
df.tree$OTU_ID <- row.names(df.tree)
write.table(df.tree, file = 'tree_backbone.txt', sep = '.', 
            col.names = F, row.names = F, quote = F)

# set color for each phylum
table.phylum <- sort(table(df.tax.filter[, 'phylum']), decreasing = T)
...
df.color <- merge(unique(df.tax.filter[, c('phylum', 'family')]),
                  df.color, by = 'phylum')
...
# generate family annotation file
families <- unique(df.tax.filter[, 'family'])
family_color <- data.frame(stringsAsFactors = F)
for (sub.family in families) {
    # name.family <- substr(sub.family, 4, nchar(sub.family))
    sub.anno <- data.frame(stringsAsFactors = F)
    sub.anno[1, 1] <- sub.family
    sub.anno[1, 2] <- 'annotation'
    sub.anno[1, 3] <- '*'
    # rotate text 90 degree
    sub.anno[2, 1] <- sub.family
    sub.anno[2, 2] <- 'annotation_rotation'
    sub.anno[2, 3] <- '90'
    # set background color
    sub.anno[3, 1] <- sub.family
    sub.anno[3, 2] <- 'annotation_background_color'
    sub.anno[3, 3] <- df.color[df.color$family == sub.family, 'color']
    
    family_color <- rbind(family_color, sub.anno)
    
}

label_color <- rbind(phylum_color, family_color)
