...
def sub_cre(df_all, path_tmp, dict_in):
    file_index = dict_in['file_index']
    path_cre = dict_in['path_cre']
    ...
    df_index_dhs = pd.merge(
        df_all, df_index, left_index=True, right_on='dhs_idx')
    df_cre = pd.read_csv(
        file_cre, sep='\t', header=None, usecols=[3, 4],
        names=['dhs_id_term', 'type_cre'])
    df_index_dhs_cre = pd.merge(
        df_index_dhs, df_cre, on='dhs_id_term')
    df_index_dhs_cre = df_index_dhs_cre.sort_index()
    df_index_dhs_cre.to_csv(os.path.join(path_cre, 'dhs_cRE.txt'),
                            sep='\t', index=None, na_rep='.')
    df_dhs_cre = df_index_dhs_cre.loc[:, ['dhs_idx', 'type_cre']]

    def calculate_score(df_in):
        if df_in.shape[0] == 1:
            return df_in
        else:
            type_cre = ','.join(df_in['type_cre'].tolist())
            row_out = pd.DataFrame(
                {'dhs_idx': df_in.iloc[0, 0], 'type_cre': type_cre,
                 'key': df_in.iloc[0, 2]})
            return row_out

    df_dhs_cre['key'] = df_dhs_cre['dhs_idx']
    print(term)
    df_dhs_cre = df_dhs_cre.fillna('.')
    df_dhs_uniq = df_dhs_cre.groupby('key').apply(calculate_score)
    df_dhs_uniq = df_dhs_uniq.drop_duplicates()
    df_dhs_uniq.index = df_dhs_uniq['dhs_idx']
    df_out = pd.merge(
        df_all, df_dhs_uniq, left_index=True, right_index=True, how='left')
    ...
    # df_out = df_out.sort_values('dhs_idx')
    file_tmp = os.path.join(path_tmp, str_term + '.txt')
    df_out = df_out.rename(columns={'type_cre': term})
    if df_out.shape[0] != df_all.shape[0]:
        print(term)
        print(df_out.shape[0])
        print(df_all.shape[0])
...
def cre_matrix():
    df_all = pd.read_csv(
        file_all_index, sep='\t', header=None, usecols=[3], names=['dhs_id'])
    path_tmp = os.path.join(path_matrix, 'tmp_cRE')
    if not os.path.exists(path_tmp):
        os.mkdir(path_tmp)
    file_dhs_id = os.path.join(path_tmp, 'dhs_id.txt')
    df_all.to_csv(file_dhs_id, sep='\t', index=None, columns=['dhs_id'])
    ...
    for i in df_meta_tissue.index:
        ...
        list_input.append({'file_index': file_index,
                           'term': f"{life_organ} | {term}",
                           'str_term': f"{str_life_organ}+{str_term}",
                           'file_h3k27ac': file_cre,
                           'path_h3k27ac': path_term_cre})

    pool = Pool(num_cpu)
    func_cre = partial(sub_cre, df_all, path_tmp)
    list_res = pool.map(func_cre, list_input)
    pool.close()

    list_tmp.extend(list_res)
    file_matrix_cre = os.path.join(path_matrix, 'cRE_matrix.txt')
    os.system(f"paste {' '.join(list_tmp)} > {file_matrix_cre}")
...
if __name__ == '__main__':
    ...
    file_promoter_uniq = \
        path_root + '/origin_data/gene/' \
                    'promoters.up2k.protein.gencode.v29.unique.bed'
    df_promoter = pd.read_csv(file_promoter, sep='\t', header=None)
    df_promoter['idx'] = df_promoter.index
    df_promoter['gene'] = df_promoter[3].apply(
        lambda x: x.split('<-')[0])
