...
FeaturePlot(seurat.all.filter, 'EGR1', reduction = 'umap')
FeaturePlot(seurat.all.filter, 'HES1', reduction = 'umap')

# chondrocyte cell type
path_tsne <- paste0(path.data, 'TSNE')
if (!file.exists(path_tsne)) {
    dir.create(path_tsne)
}

Plot.TSNE <- function(seurat.all.filter, path_tsne, dim) {
    seurat.all.filter <- RunTSNE(seurat.all.filter, reduction = "harmony", dims = 1:dim)
    plot.tsne <- DimPlot(seurat.all.filter, group.by = 'celltype.abbr', 
                         label = T, reduction = 'tsne')
    ggsave(plot.tsne, path = path_tsne, 
           filename = paste0('tsne_', dim, '.png'),
...
df.res <- foreach(dim = dims) %dopar% Plot.TSNE(seurat.all.filter, path_tsne, dim)


DimPlot(seurat.all.filter, group.by = "celltype.abbr", label = T, reduction = 'tsne')
marker.4 <- FindMarkers(seurat.chon, ident.1 = '4', group.by = 'RNA_snn_res.0.3',
                         logfc.threshold = 0.5, min.diff.pct = 0.1, only.pos = T)
...
seurat.chon <- RunPCA(seurat.chon, verbose = F, npcs = 50)
seurat.chon <- RunHarmony(seurat.chon, "batch", reduction.save = "harmony2")
seurat.chon <- RunUMAP(seurat.chon, reduction = "harmony2", 
                       dims = 1:20, n_neighbors = 20, min.dist = 0.05)
seurat.chon <- RunTSNE(seurat.chon, reduction = "harmony2", 
                       dims = 1:20)

seurat.chon <- FindNeighbors(seurat.chon, reduction = "harmony", dims = 1:50)
seurat.chon <- FindClusters(seurat.chon, resolution = 2)

DimPlot(seurat.chon, group.by = 'RNA_snn_res.0.3', label = T)
DimPlot(seurat.chon, group.by = "RNA_snn_res.2", label = T, reduction = 'harmony')
DimPlot(seurat.chon, group.by = "celltype.abbr", label = T, reduction = 'umap')
FeaturePlot(seurat.chon, 'EGR1', reduction = 'harmony')
FeaturePlot(seurat.chon, 'EGR1', reduction = 'umap')
...
ggsave(plot.lineage, path = path.lineage, 
       filename = 'chon_lineage_harmony.png',
...
ggplot(data = df.pc.gene, aes(x = PC2, y = COL9A2)) + 
...
ggplot(data = df.pc.gene, aes(x = Harmony2, y = COL9A2)) + 
    geom_point(aes(color = celltype), size = 1) + 
    scale_color_manual(labels = c('CSC','TC','Chondrocyte1','Chondrocyte2'),
                       values = c("#33A02C","#B2DF8A" ,"#A6CEE3","#1F78B4")) + 
    # geom_smooth(method = 'lm', formula = y~poly(x,5), color = '#696969') + 
...
ggplot(data = df.pc.gene, aes(x = Harmony2, y = CEBPD, linetype = status)) + 
    geom_point(aes(color = celltype), size = 1) + 
    scale_color_manual(labels = c('CSC','TC','Chondrocyte1','Chondrocyte2'),
                       values = c("#33A02C","#B2DF8A" ,"#A6CEE3","#1F78B4")) + 
