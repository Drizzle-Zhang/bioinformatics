...
down.marker.CSC <- marker.CSC[marker.CSC$p_val_adj < 0.01 & 
                                  marker.CSC$avg_logFC < -0.3,]
down.gene.CSC <- c(rownames(down.marker.CSC), 'SOX5', 'DBP', 'ARID5B')
...
FCs <- abs(marker.CSC[V(igraph_down)$name, 'avg_logFC'])
norm_FCs <- FCs
...
saveRDS(igraph_down_2, file_CSC_down)
...
up.gene.CSC <- setdiff(rownames(up.marker.CSC), c('CEBPB', 'CEBPD', 'KLF2'))
RegulonInfo_CSC_up <- 
...
saveRDS(igraph_up_2, file_CSC_up)
