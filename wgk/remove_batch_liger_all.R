...
DimPlot(subset(seurat.ear.liger, subset = sample == 'C4'), group.by = "sample")
...
# k = 20 for default, large k leads to more aggressive correction
k = 20
seurat.ear.liger <- RunOptimizeALS(seurat.ear.liger, k = k, lambda = 1, split.by = "sample")
seurat.ear.liger <- RunQuantileNorm(seurat.ear.liger, split.by = "sample")
# reduction using iNMF
seurat.ear.liger <- FindNeighbors(seurat.ear.liger, reduction = "iNMF", dims = 1:k)
seurat.ear.liger <- FindClusters(seurat.ear.liger, resolution = 0.2)
seurat.ear.liger <- RunUMAP(seurat.ear.liger, dims = 1:ncol(seurat.ear.liger[["iNMF"]]), 
                            reduction = "iNMF", n.neighbors = 30)

file.liger <- '/home/disk/drizzle/wgk/data/ear.liger_4000_20_1.Rdata'
saveRDS(seurat.ear.liger, file = file.liger)
...
# cluster
table(seurat.ear.liger$sample, seurat.ear.liger$RNA_snn_res.0.2)/as.vector(table(seurat.ear.liger$sample))
mytable <- table(seurat.ear.liger$sample, seurat.ear.liger$RNA_snn_res.0.2)[,1:5]
mytable/rowSums(mytable)
