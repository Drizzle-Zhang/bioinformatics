...
path.data <- '/home/disk/drizzle/wgk/data/marker_3_merge/'
if (!file.exists(path.data)) {
    dir.create(path.data)
}
seurat.first <- FindClusters(seurat.first, resolution = 3)
DimPlot(seurat.first, group.by = "seurat_clusters", label = T)

cluster_1.5 <- seurat.first$RNA_snn_res.3
vec.cluster <- rep('_', length(cluster_1.5))
vec.cluster[cluster_1.5 %in% c(11, 16, 39)] <- '1'
vec.cluster[cluster_1.5 %in% c(13, 18, 21, 23)] <- '3'
vec.cluster[cluster_1.5 %in% c(22)] <- '5'
vec.cluster[cluster_1.5 %in% c(24, 35)] <- '7'
vec.cluster[cluster_1.5 %in% c(14, 29)] <- '12'
vec.cluster[cluster_1.5 %in% c(2, 3, 4, 6, 8, 10, 12, 19)] <- '9'
vec.cluster[cluster_1.5 %in% c(0, 1, 5, 7, 9, 15, 20, 26, 27)] <- '4'
vec.cluster[cluster_1.5 %in% c(36)] <- '11'
vec.cluster[cluster_1.5 %in% c(17, 25, 30, 31)] <- '2'
vec.cluster[cluster_1.5 %in% c(28, 32, 33, 37)] <- '6'
vec.cluster[cluster_1.5 %in% c(38)] <- '8'
vec.cluster[cluster_1.5 %in% c(34, 40)] <- '10'
table(seurat.first$sample, vec.cluster)/as.vector(table(seurat.first$sample))

seurat.first$vec.cluster <- as.numeric(vec.cluster)
DimPlot(seurat.first, group.by = "vec.cluster", label = T)
...
for (cluster in clusters) {
    ...
    list.marker[[as.character(cluster)]] <- sub.markers
}

sub.markers <- FindMarkers(seurat.first, ident.1 = c(4, 9), group.by = 'vec.cluster',
                           logfc.threshold = 0.5, min.diff.pct = 0.1, only.pos = T)
list.marker[['49']] <- sub.markers

sub.markers <- FindMarkers(seurat.first, ident.1 = c(1, 3, 5), group.by = 'vec.cluster',
                           logfc.threshold = 0.5, min.diff.pct = 0.1, only.pos = T)
list.marker[['135']] <- sub.markers

sub.markers <- FindMarkers(seurat.first, ident.1 = c(23), group.by = 'RNA_snn_res.1.5',
                           logfc.threshold = 0.5, min.diff.pct = 0.1, only.pos = T)


file.marker.first <- paste0(path.data, 'seurat_first_marker.Rdata')
...
list.go <- list()
list.kegg <- list()
clusters <- unique(names(list.marker))
for (cluster in clusters) {
...
df.geneid2symbol[unlist(strsplit((list.kegg$`5`)['hsa04550', 'geneID'], '/')),]
