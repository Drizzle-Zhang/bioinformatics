...
seurat.first <- FindClusters(seurat.first, resolution = 1.5)
DimPlot(seurat.first, group.by = "seurat_clusters", label = T)

cluster_1.5 <- seurat.first$RNA_snn_res.1.5
vec.cluster <- rep('_', length(cluster_1.5))
vec.cluster[cluster_1.5 %in% c(11, 15)] <- '1'
vec.cluster[cluster_1.5 %in% c(8, 10, 12)] <- '3'
vec.cluster[cluster_1.5 %in% c(17)] <- '5'
vec.cluster[cluster_1.5 %in% c(7, 23)] <- '7'
vec.cluster[cluster_1.5 %in% c(0, 2, 3, 4)] <- '9'
vec.cluster[cluster_1.5 %in% c(1, 5, 9, 6)] <- '4'
vec.cluster[cluster_1.5 %in% c(20)] <- '11'
vec.cluster[cluster_1.5 %in% c(13, 16, 18)] <- '2'
vec.cluster[cluster_1.5 %in% c(14, 21)] <- '6'
vec.cluster[cluster_1.5 %in% c(22)] <- '8'
vec.cluster[cluster_1.5 %in% c(19, 24)] <- '10'
