    ...
    with open(file_out, 'w') as w_out:
        ...
        with open(file_in, 'r') as r_pp:
            for i, line in enumerate(r_pp):
                ...
                loop_id = 'po' + str(i)
                chrom1 = list_line[0]
                start1 = int(list_line[1]) - 1000
                end1 = int(list_line[2]) + 1000
                chrom2 = list_line[4]
                start2 = int(list_line[5]) - 1000
                end2 = int(list_line[6]) + 1000
                if chrom1 == chrom2:
...
def annotate_hic(dict_in):
    ...
    if func_tran is None:
        ...
        transform_3div(file_hic, file_uniform, cutoff=2)
    elif func_tran == 'pcHi-C_ng2019':
        transform_ng2019(file_hic[0], file_hic[1], file_uniform, cutoff=0)
