...
if __name__ == '__main__':
    time_start = time()
    path_data_root = '/root/scATAC/ATAC_data/Forebrain/Mulqueen_human_brain_DPC'
    ...
    dataset_ATAC.select_genes(num_peak=200000)
    ...
    # sc.pp.normalize_total(adata)
    # sc.pp.log1p(adata)
    sc.pp.highly_variable_genes(adata_edge, n_top_genes=20000, flavor='seurat')
    ...
    dict_promoter = {}
    file_peak_promoter = \
        '/root/scATAC/ATAC_data/Forebrain/Mulqueen_human_brain_DPC/processed_files/peaks_promoter.txt'
    ...
    df_graph = dataset_ATAC.df_graph
    list_pairs = df_graph.apply(lambda x: (x['region1'], x['region2']), axis=1).tolist()
    # AD sites
