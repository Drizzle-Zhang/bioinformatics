...
df.plot.CV$`Mean Accuracy` <- rowMeans(df.plot.CV)
df_acc <- df.plot.CV
...
df.plot.CV$`Mean Balanced Accuracy` <- rowMeans(df.plot.CV)
df_ban_acc <- df.plot.CV

mat.plot <- cbind(df_acc, df_ban_acc)
...
sel.type <- c('Mean Accuracy', 'Mean Balanced Accuracy')
...
df.plot <- reshape2::melt(as.matrix(mat.plot))
names(df.plot) <- c('Method', 'Dataset', 'Accuracy')
vec.metrics <- c(rep('Cross validation', 50), rep('Cross platform', 70),
                 rep('Mean', 10))
...
df.plot$Method <- factor(df.plot$Method,
                         levels = df.mean$Method[order(df.mean$Accuracy, decreasing = F)])
df.plot$Method <- factor(df.plot$Method,
                         levels = rev(unique(c('scMAGIC', 
                                           as.character(df.mean$Method[order(df.mean$Accuracy, decreasing = T)])))))
color_type <- rep('0', nrow(df.plot))
