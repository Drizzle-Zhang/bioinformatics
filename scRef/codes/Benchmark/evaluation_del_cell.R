...
path.out <- '/home/zy/scRef/try_data/evaluation_del_cell/'
...
sub.evaluation <- function(exp_ref_mat.origin, data.filter, label.filter, path.out,
    ...
    for (i in 1:length(cells.ref)) {
        ...
        # out.markers <- find.markers(exp_ref_mat, cutoff.fc = 1, cutoff.pval = 0.05)
        out.markers <- find.markers(exp_ref_mat, topN = 100)
        ...
        meta.tag <- comfirm.label(exp_sc_mat, list.cell.genes, ori.tag, scRef.tag)
