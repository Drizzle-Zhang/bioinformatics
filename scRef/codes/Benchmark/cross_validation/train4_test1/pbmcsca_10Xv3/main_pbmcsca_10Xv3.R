...
setwd('/home/zy/my_git/scRef/Benchmark/cross_validation/train4_test1/pbmcsca_10Xv3')
source('./Cross_Validation.R')
source('./method_functions.R')
source('./evaluate.R')

path.input <- '/home/zy/scRef/sc_data/'
path.output <- '/home/zy/scRef/cross_validation/train4_test1/pbmcsca_10Xv3/'
if (!dir.exists(path.output)) {
    dir.create(path.output)
}

# filter data
dataset <- 'pbmcsca_10Xv3'
...
data("pbmcsca")
OUT <- list()
OUT$data.filter <- as.matrix(pbmcsca@assays$RNA@counts[, pbmcsca$Method == '10x Chromium (v3)'])
OUT$label.filter <- data.frame(annotations = as.character(pbmcsca$CellType)[pbmcsca$Method == '10x Chromium (v3)'],
