evaluate <- function(TrueLabelsPath, PredLabelsPath, del.cells){
  "
  ...
  true_lab[true_lab %in% del.cells] <- 'Unassigned'
  pred_lab <- unlist(read.csv(PredLabelsPath, stringsAsFactors = F))
  unique_cell <- setdiff(unlist(unique(true_lab)), 'Unassigned')
  pred_lab[!(pred_lab %in% unique_cell)] <- 'Unassigned'
  ...
  med_F1 <- median(F1)
  if (F1['Unassigned'] == 0) {
    mean_F1 <- mean(F1[names(F1) != 'Unassigned'])
  } else {
    mean_F1 <- mean(F1)
  }
  ...
  result <- list(Conf = conf, MedF1 = med_F1, F1 = F1, Mean_F1 = mean_F1, 
                 ...
                 Acc = acc, PercUnl = per_unlab, PopSize = pop_size)
