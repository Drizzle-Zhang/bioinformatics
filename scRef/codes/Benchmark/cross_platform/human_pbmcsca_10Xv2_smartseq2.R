...
dataset <- 'Smartseq2'
exp_sc_mat <- as.matrix(pbmcsca@assays$RNA@counts[, pbmcsca$Method == 'Smart-seq2'])
label_sc <- as.character(pbmcsca$CellType)[pbmcsca$Method == 'Smart-seq2']
median(colSums(exp_sc_mat != 0))

ref.names <- unique(ref.labels)
all.cell <- unique(label_sc)
uniform.names <-
    c("B cell", "T cell", "Dendritic cell", "T cell",
      "Megakaryocyte", "Natural killer cell", "monocyte",
      "monocyte", "Plasmacytoid dendritic cell")
df.ref.names <- data.frame(ref.name = ref.names, name = uniform.names)
uniform.names <-
    c("T cell", "T cell", "monocyte", "B cell", "Megakaryocyte", 
