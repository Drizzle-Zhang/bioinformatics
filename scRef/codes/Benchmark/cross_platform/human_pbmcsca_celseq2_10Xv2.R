...
ref.dataset <- 'CELSeq2'
ref.mtx <- as.matrix(pbmcsca@assays$RNA@counts[, pbmcsca$Method == 'CEL-Seq2'])
ref.labels <- as.character(pbmcsca$CellType)[pbmcsca$Method == 'CEL-Seq2']
...
exp_sc_mat <- exp_sc_mat[, !(label_sc %in% c('Dendritic cell', 'Plasmacytoid dendritic cell'))]
label_sc <- label_sc[!(label_sc %in% c('Dendritic cell', 'Plasmacytoid dendritic cell'))]
median(colSums(exp_sc_mat != 0))

ref.names <- unique(ref.labels)
all.cell <- unique(label_sc)
uniform.names <-
    c("T cell", "T cell", "Natural killer cell", "monocyte",
      "monocyte", "Megakaryocyte", "B cell")
df.ref.names <- data.frame(ref.name = ref.names, name = ref.names)
uniform.names <-
    c("B cell", "T cell", "T cell", "Megakaryocyte",
      "Natural killer cell", "monocyte", "monocyte")
df.sc.names <- data.frame(sc.name = all.cell, name = all.cell)

path.output <- '/mdshare/node9/zy/scRef/Benchmark/human_pbmsca/'
...
output.scMAGIC <- scMAGIC(exp_sc_mat, ref.mtx, ref.labels,
                          atlas = 'HCL',
                          corr_use_HVGene1 = 800, corr_use_HVGene2 = 800,
