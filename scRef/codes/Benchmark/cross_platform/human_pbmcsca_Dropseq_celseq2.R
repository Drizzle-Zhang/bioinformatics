...
ref.dataset <- 'Dropseq'
ref.mtx <- as.matrix(pbmcsca@assays$RNA@counts[, pbmcsca$Method == 'Drop-seq'])
ref.labels <- as.character(pbmcsca$CellType)[pbmcsca$Method == 'Drop-seq']
median(colSums(ref.mtx != 0))

############### import unlabeled data
dataset <- 'CELSeq2'
exp_sc_mat <- as.matrix(pbmcsca@assays$RNA@counts[, pbmcsca$Method == 'CEL-Seq2'])
label_sc <- as.character(pbmcsca$CellType)[pbmcsca$Method == 'CEL-Seq2']
