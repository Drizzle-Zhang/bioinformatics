...
path.out <- '/home/zy/scRef/try_data/evaluation_del_cell_scRef2/'
...
sub.evaluation <- function(exp_ref_mat.origin, data.filter, label.filter, path.out,
    ...
    for (i in 1:length(cells.ref)) {
        ...
        ######################### run scRef
        result.scref <- SCREF2(exp_sc_mat, exp_ref_mat, type_ref = 'fpkm', min_cell=5, CPU = num.cpu)
        meta.tag <- merge(result.scref$pvalue2, label.filter, by = 'row.names')
        row.names(meta.tag) <- meta.tag$Row.names
        meta.tag$Row.names <- NULL
        names(meta.tag) <- c('scRef.tag', 'pvalue', 'qvalue', 'ori.tag')
        
        pvalue1 <- result.scref$pvalue1
        names(pvalue1) <- c('scRef.tag.1', 'pvalue.1', 'qvalue.1')
        pvalue2 <- result.scref$pvalue2
        names(pvalue2) <- c('scRef.tag.2', 'pvalue.2', 'qvalue.2')
        pvalue <- merge(pvalue1, pvalue2, by = 'row.names')
        row.names(pvalue) <- pvalue$Row.names
        pvalue$Row.names <- NULL
        pvalue <- merge(pvalue, label.filter, by = 'row.names')
        list.p.combine <- apply(pvalue[, c('pvalue.1', 'pvalue.2')], 1, sumlog)
        p.combine <- c()
        for (j in 1:length(list.p.combine)) {
            sub.list <- list.p.combine[[j]]
            p.combine <- c(p.combine, sub.list$p)
        }
        pvalue$p.combine <- p.combine
        
        cor1 <- result.scref$out1
        cor2 <- result.scref$out2
        cor.cells <- intersect(dimnames(cor1)[[1]], dimnames(cor2)[[1]])
        cor1 <- cor1[cor.cells,]
        cor2 <- cor2[cor.cells,]
        cor <- cor1 + cor2
        tag <- .get_tag_max(cor)
        df.tag <- as.data.frame(tag)
        row.names(df.tag) <- df.tag$cell_id
        df.tag$cell_id <- NULL
        names(df.tag) <- 'scRef.tag'
        df.tag <- merge(df.tag, label.filter, by = 'row.names')
        
        df.pvalue.tag <- merge(pvalue, df.tag, by = 'Row.names')
        ...
        vec.cutoff <- c(1e-8, 1e-7, 1e-6, 1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 
                        seq(0.002, 0.1, 0.001), seq(0.2, 1, 0.1))
        ...
        best.cutoff <- df.metrics$cutoff[df.metrics$weighted.f1 == max(df.metrics$weighted.f1)][1]
        new.tag <- meta.tag$scRef.tag
        new.tag[meta.tag$qvalue >= default.cutoff] <- 'unknown'
        ...
        # remove neuron
        meta.tag.remove <- meta.tag[meta.tag$ori.tag != name.neuron, c("ori.tag", "scRef.tag", "qvalue")]
        vec.cutoff <- c(1e-8, 1e-7, 1e-6, 1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 
                        seq(0.002, 0.1, 0.001), seq(0.2, 1, 0.1))
        ...
        # default cutoff
        new.tag.remove <- meta.tag.remove$scRef.tag
        new.tag.remove[meta.tag.remove$qvalue >= default.cutoff] <- 'unknown'
        true.tag.remove <- meta.tag.remove$ori.tag
...
# delete specific cell
cells.ref <- c("", "Astrocyte", "Microglia", "Endothelial cell")
cells.unlabeled <- c("", "Astrocyte", "Microglia", "Endothelial Cell")
# evaluation
out <- sub.evaluation(exp_ref_mat.origin, data.filter, label.filter, path.out,
                      df.cell.names, cells.ref, cells.unlabeled, dataset)
