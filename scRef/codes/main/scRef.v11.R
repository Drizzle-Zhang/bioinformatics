...
.imoprt_outgroup <- function(out.group = 'MCA', opt.SEG = T) {
...
.find_markers_RUVseq <- function(exp_ref_mat, out.group, 
                                 type_ref = 'count', topN = 100) {
    library(parallel, verbose = F)
    library(Seurat, verbose = F)
    # check parameters
    if (!is.null(topN)) {
...
SCREF <- function(exp_sc_mat, exp_ref_mat, 
    ...
    seurat.out.group <- .imoprt_outgroup(out.group = out.group, opt.SEG = T)
    
    if (identify_unassigned) {
        # find markers of cell types in reference
        print('Find marker genes of cell types in reference:')
        out.markers <-
            .find_markers_RUVseq(exp_ref_mat, seurat.out.group, 
                                 type_ref = type_ref, topN = topN)
    ...
    if (identify_unassigned) {
        ...
        out.markers <- .find_markers_RUVseq(LocalRef, seurat.out.group, topN = 100)
