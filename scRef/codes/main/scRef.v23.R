...
.find_markers_sc_bak <- function(select.exp, vec.tag1, LocalRef, 
...
.cutoff_GMM_add_neg_bak <- function(df.tags.in, num_cluster = 6, cutoff.neg = 3, 
...
SCREF <- function(exp_sc_mat, exp_ref_mat, exp_ref_label = NULL, 
                  ...
                  cluster.speed = NULL, cluster.cell = NULL, min_cell = NULL, 
    ...
    if (cluster.speed) {
        if (is.null(cluster.cell)) {
            cluster.cell <- ceiling(num.cell/2000)
        }
        if (is.null(min_cell)) {
            min_cell <- cluster.cell * 2
        }
    } else {
        min_cell <- 1
    }
    ...
    print('Build local reference')
    df.tags2 <- .confirm_label(df.exp.merge, list.cell.genes, tag2, CPU = CPU)
    cell_ids <- colnames(df.exp.merge)
    df.tags2 <- df.tags2[cell_ids, ]
    
    out.cutoff <- .cutoff_GMM_add_neg(df.tags2, num_cluster = GMM.num_cluster, 
                                      cutoff.neg = GMM.neg_cutoff, 
                                      cutoff.pos = GMM.floor_cutoff,
                                      ceiling.cutoff = GMM.ceiling_cutoff, 
                                      opt.strict = T, opt.negative = T)
    df.cutoff.2 <- out.cutoff$vec.cutoff
    neg.cutoff.2 <- out.cutoff$vec.neg.cutoff
    print('Default cutoff: ')
    print(df.cutoff.2)
    ...
    for (cell in names(df.cutoff.2)) {
      ...
      sub.select <- df.tags2[df.tags2$scRef.tag == cell, ]
      sub.select <- sub.select[sub.select$log10Pval >= sub.cutoff, ]
      select.barcode <- c(select.barcode, row.names(sub.select))
    }
    list.localNeg <- list()
    for (cell in names(neg.cutoff.2)) {
      print(cell)
      sub.cutoff <- neg.cutoff.2[cell]
      if (is.na(sub.cutoff)) {
        next()
      }
      sub.select <- df.tags2[df.tags2$scRef.tag == cell, ]
    ...
    select.exp2 <- df.exp.merge[, cell_ids %in% select.barcode]
    select.tag2 <- tag2[tag2[, 'cell_id'] %in% select.barcode, ]
    LocalRef2 <- .generate_ref(select.exp2, select.tag2,  min_cell = min_cell)
    vec.tag2 <- select.tag2[, 'tag']
    ...
    if (identify_unassigned) {
        ...
        suppressMessages(
            out.markers <-
                .find_markers_sc(
                    select.exp2, vec.tag2, LocalRef2, 
                    seurat.out.group, list.localNeg,
                    use.RUVseq = use.RUVseq,
                    base.topN = topN,
                    percent.high.exp = percent.high.exp,
    ...
    print('Third-round annotation:')
    method3 <- 'multinomial'
    print(method3)
    if (!is.null(corr_use_HVGene2)) {
        HVG <- .get_high_variance_genes(LocalRef2, num.genes = corr_use_HVGene2)
        similarity.in <- df.exp.merge[HVG, ]
        ref.in <- LocalRef2[HVG, ]
    } else {
        similarity.in <- df.exp.merge
        ref.in <- LocalRef2
    }
    if (method3 != 'multinomial') {
        suppressMessages(
            out3 <- .get_cor(similarity.in, ref.in, method = method3, CPU = CPU)
        )
    } else {
        suppressMessages(
            out3 <- .get_log_p_sc_given_ref(similarity.in, ref.in, CPU = CPU)
        )
    }
    tag3 <- .get_tag_max(out3)
    gc()
 
    if (identify_unassigned) {
        suppressMessages(
            df.tags3 <- .confirm_label(df.exp.merge, local.cell.genes, tag3, CPU = CPU)
        )
        cell_ids <- colnames(df.exp.merge)
        df.tags3 <- df.tags3[cell_ids, ]
        gc()
    ...
    # View(df.view)
    # table(label_sc$label.unlabeled.use.cols..., df.combine$scRef.tag)
    # table(label_sc$Cluster, pred.scRef)
