...
mytable.tag[,2] <- gsub('Astroglial cell',
                        'Other cells in MCA', mytable.tag[,2])
mytable.tag[,2] <- gsub('Endothelial cell_Ly6c1 high', 'Endothelial cell', mytable.tag[,2])
mytable.tag[,2] <- gsub('Ganglion cell_Mapt high', 'Other cells in MCA', mytable.tag[,2])
mytable.tag[,2] <- gsub('Macrophage_Klf2 high', 'Macrophage', mytable.tag[,2])
...
tag_order <- c("Astrocyte", "Endothelial cell",
               "Granule neurons", "Neuron", "Schwann cell",
               "Hypothalamic ependymal cell", "Macrophage",
               "Microglia", "Oligodendrocyte precursor cell","Unassigned",
               "Other cells in MCA")
...
mydata$origin <- factor(mydata$origin,
                        levels = c("Astrocytes", "Endothelial cells",
                                   ...
                                   "PVMs & Microglia", "OPC",
                                   "Oligodendrocytes", "Mural cells", "Pars tuberalis",
                                   "Tanycytes", "VLMCs"))
...
plot.heatmap <-
    ...
    labs(fill = 'Proportion', title = 'MCA -> Mouse HArc-ME') +
    theme_bw() +
    theme(
...
uniform.names <-
    c('Astrocytes', 'Astroglial cell', 'Endothelial cells',
      'Neurons', 'Neurons', 'Ependymocytes', 'PVMs & Microglia',
      'PVMs & Microglia', 'Neurons', 'OPC', 'Neurons', 'Unassigned')
