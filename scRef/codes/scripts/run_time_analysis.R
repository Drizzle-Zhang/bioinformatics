...
file.runtime <- '/mdshare/node9/zy/scRef/run_time/ref_5000_query_2000_20000.txt'
...
# scID_output <- scid_multiclass(exp_sc_mat, mat.ref, Train_Labels[[1]])
...
vec.color <- colors[c(1, 3:11)]
file.runtime <- '/mdshare/node9/zy/scRef/run_time/ref_5000_query_2000_20000.txt'
df.time <- read.table(file.runtime, sep = '\t')
plot.time <- 
    ggplot(df.time, aes(x = num_cell, y = run_time, color = method)) + 
    geom_line() +
    scale_y_log10() + 
    labs(x = 'Number of cells', y = 'RunTime (min)', color = 'Method') + 
...
ggsave(filename = 'RunTime.png',
       path = '/mdshare/node9/zy/scRef/run_time', plot = plot.time,
