...
output.scMAGIC <- scMAGIC(exp_sc_mat, HCL_ref, type_ref = 'sum-counts', atlas = 'HCL',
                          opt_speed = T, combine_num_cell = 3, cluster_resolution = 1,
                          GMM.floor_cutoff = 10, GMM.ceiling_cutoff = 30,
                          min_cell = 9, use_RUVseq = F,  num_threads = 10)
time2 <- Sys.time()
...
mytable.tag[,2] <- gsub("B cell(Centrocyte)", 'B cell', mytable.tag[,2], fixed = T)
mytable.tag[,2] <- gsub('CD4-T cell', 'CD4+ T cell', mytable.tag[,2])
mytable.tag[,2] <- gsub('CD8-T cell', 'CD8+ T cell', mytable.tag[,2])
mytable.tag[,2] <- gsub('Macrophage-FCGR3A high', 'Macrophage', mytable.tag[,2])
mytable.tag[,2] <- gsub('Monocyte-IGHG4 high', 'Monocyte', mytable.tag[,2])
mytable.tag[,2] <- gsub('Proliferating  B cell', 'B cell', mytable.tag[,2])
mytable.tag[,2] <- gsub('T cell-GNLY high', 'Unassigned', mytable.tag[,2])
...
for (label1 in rownames(mytable.sum)) {
    ...
    for (label2 in unique(c(colnames(mytable.sum), 'Other cells in HCL','Unassigned'))) {
...
tag_order <- c("B cell", "CD4+ T cell",
               "CD8+ T cell", "T cell-GNLY high", 
...
uniform.names <-
    c('B cell', 'T cell', 'T cell',
      'Dendritic cell', 'Megakaryocyte', 'Monocyte',
      'Dendritic cell', 'Dendritic cell', 'Unassigned')
df.ref.names <- data.frame(ref.name = ref.names, name = uniform.names)
uniform.names <-
    c("B cell", "Dendritic cell", "Megakaryocyte", "Monocyte",
      "NK cell", "T cell")
